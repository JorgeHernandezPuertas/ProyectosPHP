...
const DIR_SERV = "./servicios/servicios_rest"
...
function cerrar_sesion () {
    $.ajax({
        ...
        .done(function (data) {
            ...
            localStorage.clear();
            window.location.href = "index.html";//salto a index.html
        })
        .fail(function (a, b) {
            $('#errores').html(error_ajax_jquery(a, b));
            $('#respuesta').html("");
...
function cargar_vista_oportuna (datos_usu_log) {
    $('#errores').html("");
    var html_respuesta = "<div id='saludo'>Bienvenido <strong>" + datos_usu_log["usuario"] + "</strong> - <button onclick='cerrar_sesion();' class='enlace'>Salir</button></div>";
    html_respuesta += "<div id='respuestas'></div><div id='productos'></div>";
...
function cargar_vista_login (error) {
    html_form_login = "<div id='login' class='centrado'><form onsubmit='event.preventDefault();hacer_login();'>";
    html_form_login += "<p><label for='usuario'>Usuario: </label><input type='text' id='usuario' required/></p>";
    html_form_login += "<p><label for='clave'>Contraseña: </label><input onkeyup='cifrar_clave();' type='password' id='clave' required/><input id='clave2' type='hidden'/></p>";
    html_form_login += "<p class='centrado'><button>Login</button></p>";
    html_form_login += "<p id='error_login' class='centrado error'>" + error + "</p>";
    html_form_login += "</form></div>";

    $('#respuesta').html(html_form_login);
}
