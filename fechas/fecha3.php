...
if (isset($_POST["btnEnviar"])) {
    ...
    $error_fecha2_vacia = $_POST["fecha1"] = "";
    $errores = $error_fecha1_vacia || $error_fecha2_vacia;

    if (!$error_fecha1_vacia){
        $array_fecha1 = explode("-", $_POST["fecha1"]);
        $fecha1 = date("m/d/Y", strtotime($array_fecha1[1]."/".$array_fecha1[2]."/".$array_fecha1[0]));
    }
    
    if (!$error_fecha2_vacia){
        $array_fecha2 = explode("-", $_POST["fecha2"]);
        $fecha2 = date("m/d/Y", strtotime($array_fecha2[1]."/".$array_fecha2[2]."/".$array_fecha2[0]));
    }
