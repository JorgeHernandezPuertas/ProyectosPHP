    ...
    if (isset($_POST["btnContInsertar"])) // compruebo errores
    {
        ...
        if (!$error_usuario) {
            ...
            if (is_string($error_usuario))
                $conexion = null;
            die(error_page("Error consultando la BD", "<p>$error_usuario</p>"));
        }
        ...
        if (!$error_email) {
            if (!isset($conexion)) {
                try {
                    $conexion = new PDO("host=" . SERVIDOR_BD . ";dbname=" . NOMBRE_BD, USUARIO_BD, CLAVE_BD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
                } catch (PDOException $e) {
                    die(error_page("Práctica 1º CRUD", "<h1>Práctica 1º CRUD</h1><p>No he podido conectarse a la base de batos: " . $e->getMessage() . "</p>"));
                }
            }
            ...
            if (is_string($error_email))
                die(error_page("Error consultando la BD", "<p>$error_email</p>"));
        }
        ...
        if (!$error_form) {
            try {
                ...
                $clave_encriptada = md5($_POST["psw"]);
